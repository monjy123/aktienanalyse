...
def search_isin(isin):
    ...
    data = api_request("/stable/search-isin", {"isin": isin}, rate_limit=0.1)
...
def get_product_segments(ticker, period="annual"):
    ...
    return api_request("/stable/revenue-product-segmentation", {
        "symbol": ticker,
        "period": period
    }, rate_limit=0.1)
...
def get_geographic_segments(ticker, period="annual"):
    ...
    return api_request("/stable/revenue-geographic-segmentation", {
        "symbol": ticker,
        "period": period
    }, rate_limit=0.1)
